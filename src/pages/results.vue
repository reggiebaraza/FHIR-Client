<template>
  <div class="container-fluid">
    ...
    <form @submit.prevent="onNewSearch" class="row mt-5">
    ...
    <div class="row mt-4">
      <div class="col-8 offset-2">
        <h5 v-if="patients">Record found!</h5>
        <h5 v-else-if="!isLoading">No patient record found</h5>
        ...
        <div
          v-else
          class="d-flex justify-content-center qtr-height align-items-center"
        >
...
export default {
  ...
  created() {
    this.fetchResults(this.$route.params.searchTerm);
  },
  methods: {
    async fetchResults(searchTerm) {
      this.isLoading = true;
      this.patients = [];
      const response = await this.$axios.get("/Patient?name=" + searchTerm);
      this.isLoading = false;
      console.log("results after search", response.data);
      this.patients = response.data.entry;
    },
    onNewSearch() {
      this.fetchResults(this.newSearchTerm);
    },
